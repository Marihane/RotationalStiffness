# app.py — Rotational stiffness (simple, readable version)
...
def choose_auto_window(rot, mom, p_min=5, p_max=15, r2_min=0.995, min_pts=8):
    """
    Try p = 5..15% and pick the LARGEST p with:
      - at least min_pts points
      - R² >= r2_min
    If none pass, pick the p (in 5..15) with the highest R² (≥ 3 points).
    Returns a dict {p, k, r2, mask}.
    """
    best = None
    fallback = None
    for p in range(p_min, p_max + 1):
        ...
        if k is None:
            continue
        # keep the best that meets the criteria (prefer largest p)
        if n >= min_pts and r2 >= r2_min:
            best = {"p": p, "k": k, "r2": r2, "mask": mask}
        # remember the best R² as fallback (needs at least 3 points)
        if n >= 3:
            if (fallback is None) or (r2 > fallback["r2"]):
...
with st.sidebar:
    ...
    mode = st.radio("Initial stiffness window", ["Auto (5–15%)", "Manual p%"], index=0)
    if mode == "Manual p%":
        p_manual = st.slider("p% of peak M", 5, 20, 10, 1)
...
# ----------------- Load data (assume 2 columns) -----------------
df = pd.read_excel(uploaded)          # assume valid file
x = df.iloc[:, 0].to_numpy(float)     # Rotation
y = df.iloc[:, 1].to_numpy(float)     # Moment

# Basic tidy: drop NaNs, sort by rotation
...
# ----------------- Compute Sj,ini -----------------
if mode == "Auto (5–15%)":
    choice = choose_auto_window(x, y, p_min=5, p_max=15, r2_min=0.995, min_pts=8)
    if choice is None:
...
x_limit = np.nanmin([v for v in [x_end_ini, x_end_sj, x_mrd if x_mrd is not None else x_max, x_max] if np.isfinite(v)])
...
ax.plot(x_line, y_line_ini, "--", color="blue",  label=f"Sj,ini ≈ {Sj_ini:.1f} kNm/rad ({note})")
